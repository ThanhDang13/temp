...
public class RequestFilter {
    ...
    /**
     * Support Filtering Spring like. Filters are implemented VertxGenericFilter interface
     * Requests comes to filters first. This method sorts filter by VertxGenericFilter.order()
     * and bind VertxGenericFilter.mappingUrls() into Router for internal processing.
     * <p>
     * After all custom filter. Add a final security filter at the end in order to checking authentications
     *
     * @param router
     */
    public void bindFilters(Router router) {
        filterResolver
                .getVertxGenericFilters()
                .stream()
        ...
        List<VertxGenericFilter> securityFilter = securityProvider
                .generateSecurityFilters(vertxRestApi.getSecurityResolver());
    ...
    private void bindSingleFilter(Router router, VertxGenericFilter filter) {
        filter
                .mappingUrls()
                .forEach(request -> bindFilterRouting(router, filter, request));
    }

    /**
     * the processing method of VertxGenericFilter is doFilter/doFilterRx
     * in case doFilterRx returns Completable success then router.next() to the next filter or controller
     * or returns Completable error in case filter got some exceptions
     *
     * @param router
     * @param filter
     * @param request
     * @return
     */
    private Route bindFilterRouting(Router router, VertxGenericFilter filter,
                                    UrlPatternMatcher.Request request) {
        return routeFilterRequest(router, request)
                .handler(
                        routingContext -> filter.doFilterRx(routingContext)
                                .subscribe(() -> {
                                    routingContext.request().resume();
                                    routingContext.next();
                                }, throwable -> filterExceptionResolver
                                        .resolveException(routingContext, throwable)));
    }
