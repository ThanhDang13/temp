...
public class ParamStandardizer {
    ...
    private Map<String, Pair<Method, IParamStandardizer>> filterParamToStandardizer(IParamStandardizer clazz) {
        return Arrays.stream(clazz.getClass().getMethods())
                .filter(method -> isAnnotated(method, ParameterName.class))
                .flatMap(method -> Arrays.stream(AnnotationUtils.findAnnotation(method, ParameterName.class)
                        .value()).map(s -> Pair.of(s, Pair.of(method, clazz))))
    ...
    public String standardize(String key, String value) throws VertxSpringCoreException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new VertxSpringCoreException("fail to standardize parameter", HttpStatus.BAD_REQUEST.value());
        }
    }
}
