...
public class ParamExtractor {
  ...
  @Autowired private ParamConverterFactory paramConverterFactory;
  @Autowired private List<ControllerParamParser> controllerParamParsers;
  @Autowired private ParamStandardizer paramStandardizer;
  ...
  public void init() {
    paramParserMap =
        controllerParamParsers.stream()
            .collect(toMap(ControllerParamParser::type, Function.identity()));
  }

  public Object[] extractArguments(
      Method method, HttpServerRequest request, String body, RoutingContext routingContext)
      ...
      if (param.isAnnotationPresent(VertxRequestParam.class)) {
        ...
        values[i] =
            getRequestParamValue(
                request, routingContext.fileUploads(), param, paramName, defaultValue);
        if (paramAnnotation.required() && values[i] == null)
          throw new VertxSpringCoreException(
              "param " + paramName + " should not be null", NOT_ACCEPTABLE.value());
        ...
        if (values[i] == null)
          throw new VertxSpringCoreException(
              "param " + pathVariable.value() + " should not be null", NOT_ACCEPTABLE.value());
        ...
        throw new VertxSpringCoreException(
            "cannot bind args values, considering annotation missing or wrong");
      }
    }
  ...
  private VertxPrincipal extractPrincipal(RoutingContext routingContext) {
    if (routingContext.user() == null) return null;
  ...
  private Object getRequestParamValue(
    ...
    Optional<FileUpload> fileUpload =
        fileUploads.stream()
            .filter(file -> file.getDelegate().name().equals(paramName))
            .findFirst();
  ...
  private Object convertPathVariable(HttpServerRequest request, Parameter param, String paramName)
      throws VertxSpringCoreException {
    String paramValue = paramStandardizer.standardize(paramName, request.getParam(paramName));
    if (paramValue != null) {
      ParamConverter paramConverter = paramConverterFactory.getParamConverter(param.getType());
