...
public class VertxDeployer {
  ...
  private Single<String> deployCluster(
      VertxOptions vertxOptions, String className, DeploymentOptions deploymentOptions) {
  ...
  private Single<String> deployOnVertx(Vertx vertx, String className, DeploymentOptions opt) {
    return vertx
        .rxDeployVerticle(className, opt)
        .doOnSuccess(
            id -> {
              logger.info(
                  "Verticle {} is deployed in {} mode with \n id {}  \n and deployment options: {} \n {}",
                  className,
                  vertx.isClustered() ? "cluster" : "non cluster",
                  id,
                  opt.toJson().toString(),
                  vertx.isClustered()
                      ? "cluster options: " + vertxOptionsModel.toVertxOptions().toString()
                      : "");
