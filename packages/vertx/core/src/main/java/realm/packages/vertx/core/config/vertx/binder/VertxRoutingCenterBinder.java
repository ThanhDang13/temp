...
public class VertxRoutingCenterBinder {
  ...
  public void initRestResolvers() {
    try {
      VertxRestApiConfigurer externalConfig =
          applicationContext.getBean(VertxRestApiConfigurer.class);
  ...
  public void bind(Router router) throws Exception {
    ...
    new RequestFilter(context, vertxRestApi.getFilterExceptionResolver()).bindFilters(router);

    bindController(
        router,
        VertxRestController.class,
        vertxRestApi.getControllerResultResolver(),
        vertxRestApi.getExceptionResolver());
    bindController(
        router,
        VertxWebController.class,
        vertxRestApi.getWebControllerResultResolver(),
        vertxRestApi.getWebExceptionResolver());
  ...
  private void bindController(
    ...
    new RequestHandler(
            ...
            router,
            controllers,
            beforeControllerMapping,
            afterControllerMapping,
            beforeResolvers,
            controllerResultResolver,
            exceptionResolver)
        .bind();
  }
  ...
  private void detectDuplicatePath() throws Exception {
    ...
    for (BeanDefinition beanDef : controllers) {
      ...
      for (Method method : beanClass.getMethods()) {
        ...
        VertxRequestMapping requestMapping =
            AnnotatedElementUtils.getMergedAnnotation(method, VertxRequestMapping.class);
        String path = pathConverter.convertPathIdentity(requestMapping);
        if (requestMappings.contains(path)) {
          throw new VertxSpringCoreWaningException(
              "Duplicate path " + path, HttpStatus.EXPECTATION_FAILED.value());
        }
  ...
  private Map<String, List<Pair<Method, Object>>> scanAfter() {
    ...
    return scanner.findCandidateComponents(applicationConfigModel.getBasePackage()).stream()
        ...
        .filter(
            pair -> AnnotationUtils.findAnnotation(pair.getLeft(), VertxAfterMapping.class) != null)
        .flatMap(
            pair -> {
              // get annotation instance of method
              VertxAfterMapping anno =
                  AnnotationUtils.findAnnotation(pair.getLeft(), VertxAfterMapping.class);
  ...
  private Map<String, List<Pair<Method, Object>>> scanBefore() {
    ...
    return scanner.findCandidateComponents(applicationConfigModel.getBasePackage()).stream()
        ...
        .filter(
            pair ->
                AnnotationUtils.findAnnotation(pair.getLeft(), VertxBeforeMapping.class) != null)
        .flatMap(
            pair -> {
              // get annotation instance of method
              VertxBeforeMapping anno =
                  AnnotationUtils.findAnnotation(pair.getLeft(), VertxBeforeMapping.class);
