...
public class DefaultControllerBeforeResolver implements ControllerBeforeResolver {
  ...
  private Completable chain(
      List<Pair<Method, Object>> beforeHandlers, Map<Parameter, Object> args, int index)
      throws InvocationTargetException, IllegalAccessException {
    Pair<Method, Object> handler = beforeHandlers.get(index);
    Completable completable =
        (Completable)
            handler.getLeft().invoke(handler.getRight(), args.values().toArray(new Object[0]));
    if (index == beforeHandlers.size() - 1) {
      return completable;
    } else {
      return completable.andThen(chain(beforeHandlers, args, index + 1));
    }
  }
}
