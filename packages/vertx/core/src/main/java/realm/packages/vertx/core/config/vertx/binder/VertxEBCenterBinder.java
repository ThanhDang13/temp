...
public class VertxEBCenterBinder {
  ...
  public void bind(EventBus eventBus) {
    ...
    for (BeanDefinition beanDefinition :
        scanner.findCandidateComponents(applicationConfigModel.getBasePackage())) {
  ...
  private void bindEB(EventBus eventBus, Object beanObj, Method method) {
    VertxEBMapping ebMapping =
        AnnotatedElementUtils.getMergedAnnotation(method, VertxEBMapping.class);

    eventBus
        .consumer(ebMapping.path())
        .toObservable()
        .subscribe(
            message -> {
              try {
                ...
                if (returnValue instanceof Maybe) {
                  ...
                  maybe.subscribe(
                      result ->
                          consumerEBResultResolver.resolve(
                              message, result), // return result back to sender
                      throwable ->
                          exceptionResolver.resolveException(
                              message, throwable), // handle exception
                      () ->
                          consumerEBCompleteResolver.resolve(
                              message, null)); // handle complete but no result returned
                } else {
                  exceptionResolver.resolveException(
                      message,
                      new VertxSpringCoreException("return value from event bus should be Maybe"));
                }
              } catch (Exception e) {
