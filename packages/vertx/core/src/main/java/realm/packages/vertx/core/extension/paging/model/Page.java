...
public class Page<T> {
  ...
  public List<T> getPagableList(Pageable pageable, List<T> objects) {
    int from =
        (pageable.getPage() - 1) * pageable.getLimit() < 0
            ? 0
            : (pageable.getPage() - 1) * pageable.getLimit();
    int to =
        (from + pageable.getLimit()) > objects.size()
            ? objects.size()
            : (from + pageable.getLimit());
