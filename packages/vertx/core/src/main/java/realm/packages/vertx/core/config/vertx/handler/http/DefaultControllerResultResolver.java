...
public class DefaultControllerResultResolver implements ControllerResultResolver {
        ...
        if (returnValue instanceof Single) {
            ...
            asyncResult
                    ...
                    .subscribe(
                            ...
                            throwable -> vertxRestApi.getExceptionResolver()
                                    .resolveException(ioWrapper.getRoutingContext(), throwable));
        } else {
            vertxRestApi.getExceptionResolver()
                    .resolveException(ioWrapper.getRoutingContext(),
                            new VertxSpringCoreException("unsupported return type (not Single)"));
        }
    }

    private Optional<Object> handleResponse(HttpServerResponse response, ResponseEntity responseEntity) throws Exception {
    ...
    protected void nonNullBodyResponse(HttpServerResponse response, Object responseBody, ResponseEntity responseEntity) throws JsonProcessingException {
        if (!(responseBody instanceof String)) {
            responseBody = DefaultJackson.objectMapper().writeValueAsString(responseBody);
        }

        Iterator<Map.Entry<String, String>> iter = responseEntity.getHeaders().toSingleValueMap().entrySet().iterator();
        while (iter.hasNext()) {
    ...
    protected void nullBodyResponse(HttpServerResponse response) {
        response.setStatusCode(NOT_FOUND.value())
                .putHeader(CONTENT_TYPE.toString(), APPLICATION_JSON)
                .end();
    }
