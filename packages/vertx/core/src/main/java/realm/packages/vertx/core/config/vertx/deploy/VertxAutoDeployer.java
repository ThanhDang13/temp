...
public class VertxAutoDeployer {
  ...
  public void searchForDeployment() {
    ...
    Set<BeanDefinition> annotatedComponents =
    ...
    if (!list.isEmpty()) {
      try {
        EnableAutoDeployVerticle autoDeploy =
            Class.forName(list.get(0).getBeanClassName())
                .getAnnotation(EnableAutoDeployVerticle.class);
